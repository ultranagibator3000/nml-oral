      ...
      <header className="sticky absolute dark:bg-black bg-white inset-x-0 top-0 z-50">
        <nav aria-label="Global" className="flex items-center justify-between p-6 lg:px-8">
          <div className="flex lg:flex-1">
            <a href="./council" className="-m-1.5 p-1.5 flex flex-row gap-2">
              <img
                alt="Naiza Logo"
          ...
          <div className="hidden text-black lg:flex lg:gap-x-12">
            {navigation.map((item) => (
              <a key={item.name} href={item.href} className={`${unbounded_semibold.className} antialiased hover:text-gray-500 dark:text-yellow-400 dark:hover:text-gray-500 text-base font-bold leading-6`}>
                {item.name}
              </a>
            ))}
          </div>
